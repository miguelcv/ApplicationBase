...
public class Mu {
	...
	public Mu(String... args) {
		...
		initializeStdLib(main);
		for (Entry<String, Object> entry : arguments.entrySet()) {
			main.define(entry.getKey(),
					new Result(entry.getValue(), Interpreter.typeFromClass(entry.getValue().getClass())), false, true);
		}
	...
	void initializeStdLib(Environment env) {
		/* standard types */
		env.define("None", new Result(Type.None, Type.Type), false, true);
		env.define("Void", new Result(Type.Void, Type.Type), false, true);
		env.define("Any", new Result(Type.Any, Type.Type), false, true);
		env.define("Bool", new Result(Type.Bool, Type.Type), false, true);
		env.define("Int", new Result(Type.Int, Type.Type), false, true);
		env.define("Real", new Result(Type.Real, Type.Type), false, true);
	...
	public Result evalFile(File path, Environment env) {
		try {
			...
			initializeStdLib(env);
			// encapsulate in void function
			String header = "fun " + funcname + "(*): (\n";
			...
			Interpreter interpreter = new Interpreter(env, this);
			Result result = null;
	...
	void error(Exception e) {
		...
		if (e instanceof MuException) {
			...
			System.err.println("Scan error at: " + se.line + " (" + new String(Character.toChars(se.cp)) + "): "
					+ se.getMessage());
		} else {

...
public class BigInteger extends Number implements Comparable<BigInteger> {
    ...
    */
    public BigInteger nextProbablePrime() {
    	...
    	if(isPosInf) throw new ArithmeticException("INF");
    	if(isNegInf) throw new ArithmeticException("-INF");
    ...
    public static final BigInteger NAN = new BigInteger(Double.NaN);
    public static final BigInteger POSITIVE_INFINITY = new BigInteger(Double.POSITIVE_INFINITY);
    public static final BigInteger NEGATIVE_INFINITY = new BigInteger(Double.NEGATIVE_INFINITY);
    ...
    public BigInteger add(BigInteger val) {
    	if(isNaN || val.isNaN()) return NAN;
    	if(isPosInf) {
    		if(val.isNegInf) return NAN;
    		return POSITIVE_INFINITY;
    	}
    	if(isNegInf) {
    		if(val.isPosInf) return NAN;
    ...
    public BigInteger multiply(BigInteger val) {
    	if(isNaN || val.isNaN()) return NAN;
    	if(isPosInf) {
    		if(val.isNegInf) return NEGATIVE_INFINITY;
    		if(val.signum == 0) return NAN;
    		if(val.signum == -1) return NEGATIVE_INFINITY;
    		return POSITIVE_INFINITY;
    	}
    	if(isNegInf) {
    		if(val.isNegInf) return POSITIVE_INFINITY;
    		if(val.signum == 0) return NAN;
    		if(val.signum == -1) return POSITIVE_INFINITY;
    		return NEGATIVE_INFINITY;
    	}
    ...
    public BigInteger divide(BigInteger val) {
    	if(isNaN || val.isNaN()) return NAN;
    	if(isPosInf) {
    		if(val.isNegInf || val.isPosInf) return NAN;
    		if(signum == -1) return NEGATIVE_INFINITY;
    		return POSITIVE_INFINITY;
    	}
    	if(isNegInf) {
    		if(val.isNegInf || val.isPosInf) return NAN;
    		if(signum == -1) return POSITIVE_INFINITY;
    		return NEGATIVE_INFINITY;
    	}

    	if(val.signum == 0) {
    		return signum==-1 ? NEGATIVE_INFINITY : signum == 1 ? POSITIVE_INFINITY : NAN; 
    	}
    ...
    public BigInteger[] divideAndRemainder(BigInteger val) {
    	if(isNaN || val.isNaN()) return new BigInteger[] {NAN, NAN};
    	if(isPosInf) {
    		if(val.isNegInf || val.isPosInf) return new BigInteger[] {NAN, NAN};
    		if(signum == -1) return new BigInteger[] {NEGATIVE_INFINITY, NAN};
    		return new BigInteger[] {POSITIVE_INFINITY, NAN};
    	}
    	if(isNegInf) {
    		if(val.isNegInf || val.isPosInf) return new BigInteger[] {NAN, NAN};
    		if(signum == -1) return new BigInteger[] {POSITIVE_INFINITY, NAN};
    		return new BigInteger[] {NEGATIVE_INFINITY, NAN};
    	}

    	if(val.signum == 0) {
    		return signum==-1 ? new BigInteger[] {NEGATIVE_INFINITY, NAN} : 
    			signum == 1 ? new BigInteger[] {POSITIVE_INFINITY, NAN} :
    				new BigInteger[] {NAN, NAN}; 
    	}
    ...
    public BigInteger remainder(BigInteger val) {
    	if(isNaN || val.isNaN) return NAN;
    	if(isPosInf) return POSITIVE_INFINITY;
    	if(isNegInf) return NEGATIVE_INFINITY;
    ...
    public BigInteger pow(BigInteger exp) {
		...
		if(exp.equals(ONE)) return this;
    	if(signum==0) return (exp.signum == 1) ? ZERO : POSITIVE_INFINITY;
    	if(isPosInf || isNegInf) return exp.signum >= 0 ? POSITIVE_INFINITY : ZERO;
    	if(abs().equals(ONE) && (exp.isPosInf || exp.isNegInf)) return NAN;
    	if(exp.isPosInf) return POSITIVE_INFINITY;
    	if(exp.isNegInf) return ZERO;

    	int exponent = exp.intValue();
    	if (exponent < 0) {
    ...
    public BigInteger abs() {
    	if(isNaN) return NAN;
    	if(isPosInf || isNegInf) return POSITIVE_INFINITY;
    ...
    public BigInteger negate() {
    	if(isNaN) return NAN;
    	if(isPosInf) return NEGATIVE_INFINITY;
    	if(isNegInf) return POSITIVE_INFINITY;
    ...
    public int compareTo(BigInteger val) {
    	if(isNaN || val.isNaN) throw new ArithmeticException("Nothing compares to NaN");
    	if(isPosInf) {
    		if(val.isPosInf) return 0;
    		return 1;
    	}
    	if(isNegInf) {
    		if(val.isNegInf) return 0;
    		return -1;
    	}
    ...
    public boolean equals(Object x) {
    	...
    	if(isPosInf) {
    		if(x instanceof BigInteger) {
    			return ((BigInteger) x).isPosInf;
    		}
    		return false;
    	}
    	if(isNegInf) {
    		if(x instanceof BigInteger) {
    			return ((BigInteger) x).isNegInf;
    		}
    		return false;
    	}
     ...
     */
    public int hashCode() {
    	if(isNaN) return "NaN".hashCode();
    	if(isPosInf) return "INF".hashCode();
    	if(isNegInf) return "-INF".hashCode();
    ...
    public String toString(int radix) {
    	if(isNaN) return "NaN";
    	if(isPosInf) return "INF";
    	if(isNegInf) return "-INF";
    ...
    private static BigInteger getRadixConversionCache(int radix, int exponent) {
        ...
        for (int i = oldLength; i <= exponent; i++) {
            cacheLine[i] = cacheLine[i - 1].pow(TWO);
        }
    ...
    public double doubleValue() {
    	...
    	if(isPosInf) return Double.POSITIVE_INFINITY;
    	if(isNegInf) return Double.NEGATIVE_INFINITY;
